...
public class SubjectDetailServlet extends HttpServlet {
	...
	protected void doGet(
		...
		out.println("<html><head><title>과목상세정보</title>"
				...
				+ "div { width : 600px; margin-left : 25%;} "
				...
				+ "#a1 { text-align : center; text-decoration: none; color : red;}"
		...
		try{
			...
			out.println("<tr><th id='no'>번호</th><td>" 
			+ subject.getNo() +"</td></tr>");
			
			out.println("<tr><th>과목명</th><td>" +
			subject.getTitle() +"</td></tr>");
			
			out.println("<tr><th>내용</th><td><textarea rows='5' cols='60'>"
			...
			out.println(
					"<a id = 'a1' href='delete.bit?no=" + subject.getNo() + "'>삭제</a>");
			out.println(
					"<a id = 'a' href='update.bit?no=" + subject.getNo() + "'>변경</a>");
